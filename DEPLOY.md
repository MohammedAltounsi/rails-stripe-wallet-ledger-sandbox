...
Recommended provider: **Resend** (resend.com, free 3,000/mo). Verify `gotozaf.com`
so receipts come from `receipts@gotozaf.com` — on-brand and lands in inboxes.
Its SMTP is `smtp.resend.com`, username `resend`, password = your API key. If you
don't want to verify a domain, **Brevo** (free 300/day) sends from any address
after signup. Leave `SMTP_ADDRESS` unset and the app just skips sending (no crash).

`SOLID_QUEUE_IN_PUMA=true` (already in `render.yaml`) runs the job worker inside
the web process, so `deliver_later` actually sends on the free single-service plan.
