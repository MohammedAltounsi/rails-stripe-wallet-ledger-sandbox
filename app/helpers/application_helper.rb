module ApplicationHelper
  ...
  def drink_wash(color)
    c = safe_hex(color)
    "background: radial-gradient(120% 100% at 30% 15%, #{c}33, transparent 62%), linear-gradient(160deg, rgba(255,255,255,.03), rgba(0,0,0,.18));"
  end

  # Only allow a real hex colour into raw SVG/CSS strings; fall back otherwise.
  # Stops any future editable product.color from injecting markup.
  def safe_hex(color)
    color.to_s.match?(/\A#[0-9a-fA-F]{3,8}\z/) ? color : "#c9a27a"
  end
    ...
    img =
      if product.image_url.present?
        tag.img(src: product.image_url, alt: product.name, loading: "lazy",
                ...
                data: { controller: "image-fallback", action: "error->image-fallback#handle" })
      end
